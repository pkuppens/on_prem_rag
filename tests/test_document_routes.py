...
def test_documents_route_has_no_core_embeddings_import():
    ...
    module = sys.modules.get("backend.query_service.api.documents")
    if module is None:
        import importlib

        module = importlib.import_module("backend.query_service.api.documents")
...
async def test_upload_triggers_service_background_task(tmp_path):
    ...
    """
    with (
        patch("backend.query_service.api.documents.document_processing_service") as mock_service,
        patch("backend.query_service.api.documents.vector_store_manager") as mock_vsm,
    ):
...
def test_list_documents_returns_files(tmp_path):
    ...
    """
    with patch("backend.query_service.api.documents.uploaded_files_dir", tmp_path):
...
def test_gdpr_article17_delete_cascades_filesystem_and_vector_store(tmp_path) -> None:
    ...
    with (
        patch("backend.query_service.api.documents.uploaded_files_dir", tmp_path),
        patch("backend.query_service.api.documents.vector_store_manager") as mock_vsm,
    ):
