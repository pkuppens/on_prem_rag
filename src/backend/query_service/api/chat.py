"""Conversational chat API endpoint for the Query Service.

Same URL paths as the original rag_pipeline/api/chat.py for backward compatibility.
Implements POST /api/v1/chat with message history and SSE streaming.
"""
...
class ChatRequest(BaseModel):
    ...
    direct: bool = Field(
        default=False,
        description="When True (stream only), skip RAG retrieval and stream LLM response directly.",
    )
...
async def chat(payload: ChatRequest) -> ChatResponse:
    ...
    try:
        ...
        chunks = orchestrator.retrieve_relevant_chunks(
            question,
            top_k=payload.top_k,
        ...
        answer = orchestrator.generate_answer(question, chunks, conversation_history=history if history else None)
    ...
    *,
    ...
    if direct:
        ...
        try:
            ...
            try:
                for token in orchestrator.generate_answer_stream(question, []):
                    yield _format_sse({"type": "token", "content": token})
            except (NotImplementedError, ValueError):
                answer = orchestrator._llm.generate(prompt)
    ...
    chunks = orchestrator.retrieve_relevant_chunks(question, top_k=top_k, similarity_threshold=similarity_threshold)
    if not chunks:
    ...
    try:
        for token in orchestrator.generate_answer_stream(question, chunks, conversation_history=history if history else None):
            yield _format_sse({"type": "token", "content": token})
