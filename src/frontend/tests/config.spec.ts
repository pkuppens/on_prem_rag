...
test.describe('API Configuration', () => {
  test('should construct correct URLs in development', () => {
    ...
    expect(apiUrls.file('test.pdf')).toBe('http://localhost:8000/api/v1/documents/test.pdf/content');

    // Test upload URL construction
    expect(apiUrls.upload('fast')).toBe('http://localhost:8000/api/v1/documents?params_name=fast');
  ...
  test('should handle URL encoding correctly', () => {
    ...
    expect(apiUrls.file(filenameWithSpaces)).toBe(
      'http://localhost:8000/api/v1/documents/my%20document.pdf/content',
    );

    // Test with special characters in parameter set
    const paramSetWithSpecialChars = 'fast&reliable';
    expect(apiUrls.upload(paramSetWithSpecialChars)).toBe(
      'http://localhost:8000/api/v1/documents?params_name=fast%26reliable',
    );
  });
});
